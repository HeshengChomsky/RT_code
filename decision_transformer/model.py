...
class ReliableVAE(nn.Module):
    def __init__(self,state_dim,act_dim,laten_dim,hiden=[60,20]):
        super().__init__()
        self.encoder=nn.Sequential()
        self.encoder.append(nn.Linear(state_dim*2+act_dim,hiden[0]))
        self.encoder.append(nn.Mish())
        for i in range(len(hiden)-1):
            self.encoder.append(
                nn.Linear(hiden[i],hiden[i+1])
            )
            self.encoder.append(nn.Mish())

        self.encoder.append(nn.Linear(hiden[-1],laten_dim))

        self.decoder=nn.Sequential(nn.Linear(laten_dim,hiden[-1]))
        self.decoder.append(nn.Mish())
        temp=len(hiden)-1
        while temp>0:
            self.decoder.append(nn.Linear(hiden[temp],hiden[temp-1]))
            self.decoder.append(nn.Mish())
            temp-=1
        self.decoder.append(nn.Linear(hiden[temp], state_dim*2+act_dim))
