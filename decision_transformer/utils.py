...
def parse():
    ...
    config_file="/mnt/f/Ubuntu/workspace/pepars/Elastic-DT-master/configs/default.yaml"
...
def reverse_discount_cumsum(x,gamma):
    disc_cumsum = np.zeros_like(x)
    disc_cumsum[0] = x[-1]
...
class EDTTrajectoryDataset(D4RLTrajectoryDataset):
    def __init__(self, dataset_path, context_len, rtg_scale, data_ratio=1.0):
        ...
        for traj in self.trajectories:
            ...
            # )
            traj["returns_to_go"] = (
                    reverse_discount_cumsum(traj["rewards"], 1.0) / rtg_scale
            )
            # done_idxs = np.argwhere(traj["terminals"]).flatten()
            # traj["reverse_terminals"]=reverse_dones(traj["terminals"],done_idxs)
        # used for input normalization
        states = np.concatenate(states, axis=0)
        self.state_mean, self.state_std = (
            np.mean(states, axis=0),
